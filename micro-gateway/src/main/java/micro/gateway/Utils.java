...
import micro.gateway.services.externos.websocket.WebSocketService;
...
public class Utils {
	...
	public LogRequest fillLog(RequestContext ctx, String token, boolean responseData) {
	...
	public void notifyAdmin(RequestContext ctx, String token, LogRequest log) {
		...
		try {
			webSocketService.sendMessage(new MessageWebsocket(data.get(0),
					"El usuario " + data.get(0) + " -> '" + mensaje + "' ", topicAdmin, request.getMethod()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
