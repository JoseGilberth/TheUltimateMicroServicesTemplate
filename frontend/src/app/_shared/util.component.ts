...
export class UtilComponent implements OnInit {
  ...
  myVal = ""; // Drop down selected value of reader 
  ...
  obtenerTipoUsuario(): string {
    ...
    let decodedToken = <TokenDecoded>helper.decodeToken(token.access_token);
  ...
  showSweetAlertTimer(titulo: string, texto: string, tipo: SweetAlertIcon, time: number) {
    Swal.fire({
      icon: tipo, title: titulo, html: texto, showLoaderOnConfirm: true, allowOutsideClick: false, timer: time
    });
  }


  /* ELIMINA DATOS VACIOS DENTRO DE UN ARREGLO */
  limpiaArreglo(datos: string[]): string[] {
    let contadorArreglo = 0;
    datos.forEach(data => {
      if (data.length <= 0) {
        datos.splice(contadorArreglo, 1);
      }
      contadorArreglo++;
    });
    return datos;
  ...
  /* COLOCA TODA LA INFORMACIÖN COMO UPERCASE */
  upperCaseArreglo(datos: string[]): string[] {
    for (let index = 0; index < datos.length; index++) {
      datos[index] = datos[index].toUpperCase();
    }
    return datos;
  }
  ...
  */
  ...
  /*
    OBTENER EL ARCHIVO DE LA HUELLA A BASE64
  */
  sampleAcquired(s, currentFormat: any): string {
    if (currentFormat == Fingerprint.SampleFormat.PngImage) {
      var samples = JSON.parse(s.samples);
      return Fingerprint.b64UrlTo64(samples[0]);
    }
