...
public class PostFilter extends ZuulFilter {
	...
	public Object run() {
		...
		log.info(String.format("%s   %s request to %s", "POST:  " , request.getMethod(), request.getRequestURL().toString()));
		...
		if( (response.getStatus() >= 200 &&  response.getStatus() < 300)) {
			...
			} else if( request.getMethod().equals("POST") && !request.getRequestURI().contains("/oauth/token") && ( !request.getQueryString().contains("page") || !request.getQueryString().contains("size") ) ) {
