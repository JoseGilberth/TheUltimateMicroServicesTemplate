...
public class MetricsDataBaseService implements IMetricsDataBaseServices {
	 ...
	 * 
		try {
			...
			metricsDTO.setDefaultTransactionIsolation(dbMetadata.getDefaultTransactionIsolation());
			...
			if (rs != null) {
				while (rs.next()) {
					catalogos.add(rs.getString(1).trim());
				}
			}
			metricsDTO.setCatalogs(catalogos);

			rs = dbMetadata.getClientInfoProperties();
			List<String> clientInfoProperties = new ArrayList<String>();
			if (rs != null) {
				while (rs.next()) {
			...
			if (rs != null) {
				while (rs.next()) {
					typeInfo.add(rs.getString(1).trim());
				}
			}
			metricsDTO.setTypeInfo(typeInfo);
