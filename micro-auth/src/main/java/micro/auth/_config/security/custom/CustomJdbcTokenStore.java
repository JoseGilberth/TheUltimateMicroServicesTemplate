...
public class CustomJdbcTokenStore implements TokenStore {
	...
	public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		...
		updateRequestOfLogin(token);
	}

	// ACTUALIZACION DE BANDERAS DEL HUELLERO
	public void updateRequestOfLogin(OAuth2AccessToken token) {
		LOG.info("//--------- START:: ACTUALIZAR CUSTOM TOKEN STORE ::START---------");
		try {
			String uuid = request.getParameter("uuid");
			if (uuid != null) {
				if (token != null) {
					if (token.getValue() != null) {
						RequestOfLogin requestOfLogin = requestOfLoginDao.findByUuid(uuid);
						requestOfLogin.setAccess_token(token.getValue());
						requestOfLoginDao.save(requestOfLogin);
					}
				}
			}
		} catch (Exception ex) {
			LOG.error("Algo salío mal al procesar el uuid: " + ex.getMessage());
		}
		LOG.info("//--------- END:: ACTUALIZAR CUSTOM TOKEN STORE ::END ---------");
	}
